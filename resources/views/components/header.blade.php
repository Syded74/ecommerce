        ...
        <div class="relative" x-data="{ dropdownOpen: false }">
            <button class="flex items-center focus:outline-none" @click="dropdownOpen = !dropdownOpen">
                ...
                @if(Auth::check() && Auth::user()->profile_photo)
                    <img src="{{ asset('storage/' . Auth::user()->profile_photo) }}" alt="User Avatar" class="ml-2 h-8 w-8 rounded-full">
                @else
                    <img src="{{ asset('path/to/default-avatar.png') }}" alt="User Avatar" class="ml-2 h-8 w-8 rounded-full">
                @endif
                <svg class="h-5 w-5 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                </svg>
            </button>
            <div x-show="dropdownOpen" @click.away="dropdownOpen = false" class="absolute right-0 mt-2 w-48 bg-white border rounded-md shadow-lg overflow-hidden z-10">
                @if(Auth::check())
                    @if(Auth::user()->hasRole('admin'))
                        <a href="{{ route('admin.admin-profile.show') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Admin Profile</a>
                    @else
                        <a href="{{ route('profile.show') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">User Profile</a>
                    @endif
